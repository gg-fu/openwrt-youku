...
unsigned int CCoAPUri_parse(char *p_uri, coap_endpoint_type *p_endpoint_type,
...
CCoAPContext *CCoAPContext_create(CCoAPInitParam *param)
{
    ...
    if (NULL != param->url) {
        ret = CCoAPUri_parse(param->url, &network_param.ep_type, host, &network_param.port);
    }
    if (COAP_SUCCESS != ret) {
        goto err;
    }
    ...
    /*CoAP network init*/
    ret = CCoAPNetwork_init(&network_param, &p_ctx->network);
    if (COAP_SUCCESS != ret) {
        goto err;
    }
    return p_ctx;
...
void CCoAPContext_free(CCoAPContext *p_ctx)
{
    ...
    CCoAPSendNode *cur, *next;
    CCoAPNetwork_deinit(&p_ctx->network);
    list_for_each_entry_safe(cur, next, &p_ctx->list.sendlist, sendlist, CCoAPSendNode) {
        if (NULL != cur) {
            if (NULL != cur->message) {
