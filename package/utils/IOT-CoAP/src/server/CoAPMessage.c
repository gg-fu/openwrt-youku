...
void CoAPMessageToken_dump(unsigned char *token, unsigned char tokenlen)
{
    ...
    for (index = 0; index < tokenlen; index++) {
        count = LITE_snprintf(ptr, total, "%02X", token[index]);
        ptr += count;
        total -= count;
    }
...
int CoAPMessage_send(CoAPContext *context, NetworkAddr *remote, CoAPMessage *message)
{
    ...
    readlen = CoAPNetwork_write(ctx->p_network, remote, buff, (unsigned int)msglen, ctx->waittime);
    COAP_DEBUG("readlen:%d,msglen:%d",readlen,msglen);
    if (msglen == readlen) {    //////*Send message success*/
	
    	COAP_DEBUG("header_code:%d,header_type:%d",message->header.code,message->header.type);
        if (CoAPReqMsg(message->header) || CoAPCONRespMsg(message->header)) {

            COAP_DEBUG("The message id %d len %d send success, add to the list", message->header.msgid, msglen);
            ...
            COAP_DEBUG("The message %d isn't CON msg, needless to be retransmitted", message->header.msgid  );
...
int CoAPMessage_cancel(CoAPContext *context, CoAPMessage *message)
{
    ...
    list_for_each_entry_safe(node, next, &ctx->sendlist.list, sendlist, CoAPSendNode) {
        ...
        if (node->header.msgid == message->header.msgid) {
            ...
            COAP_INFO("Cancel message %d from list, cur count %d", node->header.msgid, ctx->sendlist.count);
